...
function hookLoggerToMongoLogWriter(emitter, log, contextPrefix) {
    ...
    emitter.on('mongodb-oidc-plugin:outbound-http-request', (ev) => {
        log.debug?.('OIDC-PLUGIN', mongoLogId(1002000023), `${contextPrefix}-oidc`, 'Outbound HTTP request', { url: redactUrl(ev.url) });
    });
    emitter.on('mongodb-oidc-plugin:inbound-http-request', (ev) => {
        log.debug?.('OIDC-PLUGIN', mongoLogId(1002000024), `${contextPrefix}-oidc`, 'Inbound HTTP request', { url: redactUrl(ev.url) });
    });
}
