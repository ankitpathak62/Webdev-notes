...
async function completer(params, line) {
    ...
    const SHELL_COMPLETIONS = shell_api_1.signatures.ShellApi
        .attributes;
    const COLL_COMPLETIONS = shell_api_1.signatures.Collection
        .attributes;
    const DB_COMPLETIONS = shell_api_1.signatures.Database
        .attributes;
    const AGG_CURSOR_COMPLETIONS = shell_api_1.signatures.AggregationCursor
        .attributes;
    const COLL_CURSOR_COMPLETIONS = shell_api_1.signatures.Cursor
        .attributes;
    const RS_COMPLETIONS = shell_api_1.signatures.ReplicaSet
        .attributes;
    const CONFIG_COMPLETIONS = shell_api_1.signatures.ShellConfig
        .attributes;
    const SHARD_COMPLETE = shell_api_1.signatures.Shard
        .attributes;
    const SP_COMPLETIONS = shell_api_1.signatures.Streams
        .attributes;
    const SP_INSTANCE_COMPLETIONS = shell_api_1.signatures.StreamProcessor
        .attributes;
    ...
    if (splitLine.length <= 1) {
        ...
        const hits = expressions &&
            filterShellAPI(params, expressions, elToComplete, splitLine);
        return [(hits === null || hits === void 0 ? void 0 : hits.length) ? hits : [], line];
    }
    return [[], line];
}
function isAcceptable(params, entry, versionKey) {
    ...
    const isAcceptableEnvironment = !entry.env ||
        ...
        (connectionInfo.is_data_federation
            ? entry.env.includes(mongodb_constants_1.ADL)
            : connectionInfo.is_atlas || connectionInfo.is_local_atlas
                ? entry.env.includes(mongodb_constants_1.ATLAS)
...
function filterShellAPI(params, completions, prefix, split) {
    const hits = Object.keys(completions).filter((c) => {
        ...
        if ((apiVersionInfo === null || apiVersionInfo === void 0 ? void 0 : apiVersionInfo.strict) &&
