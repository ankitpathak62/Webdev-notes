...
class MongoDBOIDCPluginImpl {
    ...
    getSupportedDefaultScopes(issuer) {
        return ['openid', 'offline_access'].filter((scope) => {
            ...
            return (!Array.isArray(issuer.metadata.scopes_supported) ||
    ...
    getIssuerClass() {
        var _a, _b;
        const oidcPluginCustomHttpOptionsProvider = (urlObject, options) => {
            const url = urlObject.toString();
            this.logger.emit('mongodb-oidc-plugin:outbound-http-request', { url });
            (0, util_1.validateSecureHTTPUrl)(url, '<generic>');
            const { customHttpOptions } = this.options;
            if (customHttpOptions && typeof customHttpOptions === 'object')
                return customHttpOptions;
            else
                return customHttpOptions?.(url, options) ?? {};
        };
        class CustomIssuer extends openid_client_2.Issuer {
            constructor(...params) {
                super(...params);
                this[_a] = oidcPluginCustomHttpOptionsProvider;
                this.Client[openid_client_1.custom.http_options] = oidcPluginCustomHttpOptionsProvider;
                this.Client.prototype[openid_client_1.custom.http_options] =
                    oidcPluginCustomHttpOptionsProvider;
            }
            static async discover(issuer) {
        ...
        _a = openid_client_1.custom.http_options, _b = openid_client_1.custom.http_options;
        CustomIssuer[_b] = oidcPluginCustomHttpOptionsProvider;
        return CustomIssuer;
    }
    async getOIDCClient(state, redirectURIs) {
        const serverMetadata = state.serverOIDCMetadata;
        const makeScope = (issuer) => [
            ...new Set([
        ...
        try {
            ...
            throw new types_1.MongoDBOIDCError(`Unable to fetch issuer metadata for ${JSON.stringify(serverMetadata.issuer)}: ${(0, util_1.messageFromError)(err)}`, {
                cause: err,
            });
        }
                ...
                const browserStatePromise = allowFallbackIfFailed(new Promise((resolve, reject) => {
                    this.openBrowser({ url: localUrl, signal })
                        ...
                        const extraErrorInfo = () => browserHandle?.spawnargs
                            ...
                            : '';
                        browserHandle?.once('error', (err) => reject(new types_1.MongoDBOIDCError(`Opening browser failed with '${(0, util_1.messageFromError)(err)}'${extraErrorInfo()}`, { cause: err })));
    ...
    async initiateAuthAttempt(state, driverAbortSignal) {
        ...
        this.logger.emit('mongodb-oidc-plugin:auth-succeeded', {
            tokenType: state.currentTokenSet.set.token_type ?? null,
